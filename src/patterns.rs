...
lazy_static! {
    ...
    pub static ref PROMPT_INJECTION: Regex = Regex::new(
        r"(?i)(?:ignore\s+(?:all\s+)?(?:previous|above|all)\s+instructions?|disregard\s+(?:all\s+)?(?:previous|above|all)|forget\s+(?:all\s+)?(?:everything|previous|all|rules)|new\s+instruction|system\s*:\s*you\s+are|act\s+as|pretend\s+you\s+are|jailbreak|DAN\s+mode)"
    ).unwrap();
