...
| S-B3 | Integration tests with mock child server | **Completed** — covered by `tests/proxy_integration.rs` (real OS child + binary wrap) |
| S-B4 | Document pairing with webpuppet-rs-mcp | **Completed** — webpuppet family and fleet consumer acks recorded and verified |

### Wave C — Product quality

| ID | Work | Status |
|----|------|--------|
| S-C1 | Labeled mini-corpus + precision/recall smoke metrics | In progress |
| S-C2 | Redaction policies (structured JSON paths) | In progress |
| S-C3 | Stable 0.2.0 non-alpha after A+B | **Completed** |
